...
piloto_estrato = vector(length = 4)
# piloto_estrato_1 <- movies_d[sample(which(movies_d$estrato == 1), 12), ]
# selected values by movie Id
piloto_estrato[1] = movies_d[is.element(movies_d$id, c(31381,19254,50986,59578,33870,33467,55031,15324,50976,21749,41959,53125)), ]
# piloto_estrato_2 <- movies_d[sample(which(movies_d$estrato == 2), 12), ]
piloto_estrato[2] = movies_d[is.element(movies_d$id, c(75314,73195,71853,75686,75148,66921,73486,79470,79944,70735,70510,77416)), ]
# piloto_estrato_3 <- movies_d[sample(which(movies_d$estrato == 3), 12), ]
piloto_estrato[3] = movies_d[is.element(movies_d$id, c(86250,93058,95327,81505,88247,86879,82096,83658,87544,80678,87843,88763)), ]
# piloto_estrato_4 <- movies_d[sample(which(movies_d$estrato == 4), 12), ]
piloto_estrato[4] = movies_d[is.element(movies_d$id, c(5074352,338013,2106476,1675434,266543,3783958,2267998,245712,198781,120586,1832382,986264)), ]
